...
initializePosition | - | 使用给定参数重置（覆盖）AGV 的位姿。 | 是 | mobileRobotPosition.x<br>mobileRobotPosition.y<br>mobileRobotPosition.theta<br>mobileRobotPosition.mapId<br>lastNodeId<br>maps | 是 | 是<br>（电梯） | 否 | 否
...
detectObject | - | AGV 检测对象（例如负载、充电点、空闲停车位）。 | 是 | - | 否 | 是 | 是 | 是
...
| detectObject | objectType | string | 是 | 对象类型（如 load、chargingSpot、parkingPosition） |
