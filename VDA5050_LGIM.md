...
| `requests` | AGV | RCS | AGV向平台请求路径等。如果 5 秒未收到平台回复，设备应重新上报。***临工智科新增*** | 可选 |
...
| *autoPlan* | | boolean | "true"：平台要求设备自主规划此边的具体轨迹。<br>"false"：设备遵循平台下发的轨迹行驶。<br>默认值："false"。***临工智科新增*** |
...
| *orderId* | string | 当前请求关联的订单 ID。可选。 |
| *orderUpdateId* | uint32 | 当前请求关联的订单更新 ID。可选。 |
...
| ***requestStates [requestState]*** | | array | AGV 请求状态数组。包含每个请求的当前状态。<br>收到平台新路径后清除已完成或失败的状态；平台下发 cancelOrder 或完全执行新 order 时清空此数组。***临工智科新增*** |
...
| 对象结构 | 数据类型 | 描述 |
|---|---|---|
| **requestState** { | JSON 对象 | 设备请求状态对象。 |
| requestId | string | 请求唯一标识符。 |
| requestStatus | string | 请求状态，枚举：<br>'REQUESTING'：请求中，平台尚未答复，<br>'EXECUTING'：执行中，平台已答复，<br>'PAUSED'：已暂停，平台下发暂停指令，<br>'FINISHED'：已完成，<br>'FAILED'：已失败。***临工智科新增*** |
| } | | |

> 表 52：requestState 对象字段说明。***临工智科新增***
...
> 表 56：physicalParameters 对象字段说明
...
> 表 57：mobileRobotGeometry 对象字段说明
...
> 表 59：mobileRobotConfiguration 对象字段说明
...
自主导航功能是指设备在特定场景下自主规划路径而非完全依赖平台下发的固定轨迹。适用于设备上点（不在路径点上需要移动到路径点）、识别货架/栈板/充电桩、绕障、上轨等需要设备根据实际环境灵活调整路径的场景。

## 8.1 触发条件

设备进入自主导航有以下几种触发方式：

1. **平台指定**：平台下发的 `edge` 中 `autoPlan` 字段为 `true`，表示平台要求设备自主规划该边的具体轨迹。适用于设备不在点上需要上点的情况。
2. **设备主动申请**：设备在执行过程中遇到障碍需要绕行、执行识别动作（`detectObject`）等场景时，可主动向平台申请自主规划路径。

## 8.2 流程说明

### 8.2.1 设备进入自主导航状态

当设备需要自主规划路径时，应将 `state` 消息中的 `autoPlan` 字段置为 `true`，并通过 `requests` 消息将自主规划的路径上报给平台。

![自主导航流程图](./assets/autonomous_navigation_flow.svg)
> 自主导航流程图

1. 设备将 `state` 中的 `autoPlan` 置为 `true`，通知平台进入自主导航状态
2. 设备通过 `requests` 消息（`requestType: autoPlanPath`）提交自主规划的路径
3. 平台评估路径是否与锁格冲突：
   - 若无冲突，通过 `response`（`GRANTED`）批准路径
   - 若有冲突，通过 `response`（`REJECTED`）拒绝并下发 `forbiddenZones`（周围设备锁格、货架锁格、禁行区等），设备避开禁行区重新规划后再次提交
4. 路径获批准后，设备按自主规划的路径行驶，持续上报 `state` 状态
5. 路径执行完毕后，设备将 `autoPlan` 置为 `false`，退出自主导航状态
### 8.2.2 设备上报路径

设备通过 `requests` 消息提交自主规划的路径，消息中包含以下内容：

- `requestId`：请求唯一标识
- `requestType`：固定为 `'autoPlanPath'`
- **`path`**：设备自主规划的路径折线点数组（`waypoint` 数组），为核心字段
- `replaceEdges`：可选，指定需要替换的平台路径边
- `orderId` / `orderUpdateId`：可选，关联的订单信息

**`path` 字段说明：**

`path` 是一个 `waypoint` 数组，每个 `waypoint` 定义路径上的一个折线点：

| 字段 | 单位 | 类型 | 说明 |
|---|---|---|---|
| x | m | float64 | 路径点 X 坐标（项目坐标系） |
| y | m | float64 | 路径点 Y 坐标（项目坐标系） |
| *theta* | rad | float64 | 设备在该点的朝向（可选） |
| *loadTheta* | rad | float64 | 负载在该点需达到的角度，地图全局角度，非车体角度（可选） |

`path` 数组从设备当前位置开始，到目标位置结束，覆盖设备期望行驶的完整路径。平台收到后评估路径是否会与已有锁格（其他设备、货架等）冲突。

### 8.2.3 状态上报

设备在自主导航过程中需持续上报状态，状态上报规则如下：

1. **autoPlan 字段**：设备进入自主导航后将 `state` 中的 `autoPlan` 置为 `true`，退出后置为 `false`。平台通过此字段判断设备当前是否处于自主导航状态。
2. **requestStates 字段**：设备通过 `state` 中的 `requestStates` 数组上报每个请求的当前状态：
   - `REQUESTING`：请求已提交，平台尚未答复
   - `EXECUTING`：请求已获批准，正在执行
   - `PAUSED`：请求已被暂停
   - `FINISHED`：请求对应的路径已执行完毕
   - `FAILED`：请求对应的路径执行失败
3. **上报频率**：自主导航过程中状态上报频率与正常状态一致——任务中 300 毫秒一次，空闲 5 秒一次。当 `requestStates` 有变化时立即上报。
4. **路径执行状态**：设备按自主规划的路径行驶时，通过 `nodeStates` 和 `edgeStates` 上报路径执行进度，与常规订单执行方式一致。
5. **错误上报**：自主导航过程中如遇到路径规划失败、无法到达目标等情况，通过 `state` 中的 `errors` 上报错误信息。

### 8.2.4 平台响应

平台收到设备上报的路径后，通过 `response` 消息进行答复：

| 响应类型 | 说明 |
|---|---|
| `GRANTED` | 批准设备自主规划的路径，设备可按此路径行驶 |
| `REJECTED` | 拒绝设备规划的路径，设备需重新规划。<br>当设备申请的路径与平台锁格冲突时，平台会将周围其他设备锁格、货架锁格、禁行区等通过 `forbiddenZones` 下发，设备需避开这些区域重新规划路径。 |
| `QUEUED` | 确认收到请求，但尚未做出决定 |

当平台拒绝设备路径时，可通过 `response` 消息中的 `forbiddenZones` 字段指定禁行区，设备重新规划路径时应避开这些区域。
...
设备认可平台下发的整体路径走向，但需要对部分边进行替换（如绕障）。此时在 `requests` 中：

- 通过 `replaceEdges` 指定需要替换的边（`edgeId`、`sequenceId`、`startNodeId`、`endNodeId`）
- 通过 `path` 提供替换后的路径折线

**方式二：完全自主规划**

设备不执行平台下发的路径（将平台路径置为失败），完全由设备自行规划新路径。此时在 `requests` 中：
...
以下情况设备退出自主导航状态，将 `state` 中的 `autoPlan` 置为 `false`：

1. 自主规划的路径执行完毕
2. 平台通过 `cancelOrder` 取消当前订单
3. 平台下发新的完整 `order`，设备重新遵循平台路径

## 8.3 边缘场景处理

### 8.3.1 路径规划失败

设备无法规划出可行路径时，应将 `state` 中的 `autoPlan` 置为 `false`，并通过 `errors` 上报路径规划失败错误，等待平台介入。

> `autoPlan` 为 `true` 时，平台不会调度设备。因此当设备需要平台介入处理时，必须将 `autoPlan` 置为 `false`。

### 8.3.2 平台长时间未响应

设备上报 `requests` 后，若平台长时间未通过 `response` 答复，设备可自行决定是否继续等待或重试。超时时间由集成阶段双方协商确定。
